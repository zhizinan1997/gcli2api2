"""
本地文件存储管理器，支持队列写入优化。
所有凭证和状态数据存储在TOML文件中，配置数据存储在另一个TOML文件中。
"""
import asyncio
import os
import json
...
class FileStorageManager:
    ...
    async def initialize(self) -> None:
        ...
        log.debug("File storage manager initialized with queue writing")
    ...
    async def close(self) -> None:
        ...
        self._initialized = False
        log.debug("File storage manager closed with queue flushed")
