...
return get_base_model_from_feature_model(model_name) in PUBLIC_API_MODELS
...
async def get_retry_no_image_enabled() -> bool:
"""Get retry if no image in response enabled setting."""
env_value = os.getenv("RETRY_NO_IMAGE_ENABLED")
if env_value:
return env_value.lower() in ("true", "1", "yes", "on")

return bool(await get_config_value("retry_no_image_enabled", False))


async def get_retry_no_image_max_attempts() -> int:
"""Get max attempts for retrying when no image is returned."""
env_value = os.getenv("RETRY_NO_IMAGE_MAX_ATTEMPTS")
if env_value:
try:
return int(env_value)
except ValueError:
...
return int(await get_config_value("retry_no_image_max_attempts", 3))


async def get_gemini_retry_if_no_image_enabled() -> bool:
"""Get retry if no image in Gemini response enabled setting."""
env_value = os.getenv("GEMINI_RETRY_IF_NO_IMAGE_ENABLED")
if env_value:
return env_value.lower() in ("true", "1", "yes", "on")

return bool(await get_config_value("gemini_retry_if_no_image_enabled", False))


async def get_gemini_retry_if_no_image_max_attempts() -> int:
"""Get max attempts for retrying Gemini when no image is returned."""
env_value = os.getenv("GEMINI_RETRY_IF_NO_IMAGE_MAX_ATTEMPTS")
if env_value:
try:
return int(env_value)
except ValueError:
